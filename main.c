...
/*   1            13 */         // 0,0 0,1 0,2 0,3 0,4 // 1,1 1,2 1,3 // 2,2 // 3,1 3,2 ,3,3 // 4,0 // 4,1 // 4,2// 4,3 4,4
/*   2  6     10  14 */
/*   3  7  9  11  15 */
/*   4  8     12  16 */
/*   5            17  그리는 함수*/
void drawSandglass(int num){
    int index = 1;
    int sandArray[num][num];
    for (int row = 0; row < num; row++){
        for(int cal = row; cal<num + (-2 * row); cal++) {
...
void draw_kbh(){
    ...
    while (1) {
        if(num==EXIT_FLAG){     //0 일경우 출력
            printf("-----------------------------\n");
            printf("♥프로그램을 종료합니다. 이름 : 김병희 \n");
            printf("-----------------------------\n");
            return;               // 함수 끝
        }else if(num>LIMIT_NUM || num < LIMIT_NUM){     //5~10의 값이 아닐경우 제한
            printf("--------------------------------------------- \n");
